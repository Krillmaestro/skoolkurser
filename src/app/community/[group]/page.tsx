...
import { COMMUNITY_GROUPS, getCommunity, getPosts, PAGE_SIZE } from "@/lib/community";

export function generateStaticParams() {
  return COMMUNITY_GROUPS.map((group) => ({ group }));
}

export async function generateMetadata({ params }: { params: Promise<{ group: string }> }) {
  const { group } = await params;
  const name = getCommunity(group)?.group.name || "Community";
...
export default async function CommunityPage({
  ...
  const posts = getPosts(group, labelId || undefined);
  ...
  const qs = (over: Record<string, string | number | undefined>) => {
    ...
    return s ? `/community/${group}?${s}` : `/community/${group}`;
  };
      ...
      <main className="max-w-[1100px] mx-auto px-6 py-6 flex gap-6">
        <div className="min-w-0 flex-1">
          ...
          <div className="space-y-3">
            {slice.map((post) => (
              <PostCard key={post.id} group={group} post={post} label={labelById.get(post.labelId) || null} />
            ))}
          </div>
