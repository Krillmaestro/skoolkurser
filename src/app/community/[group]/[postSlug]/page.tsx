...
import { COMMUNITY_GROUPS, countComments, getCommunity, getLabel, getPost, timeAgo } from "@/lib/community";

export function generateStaticParams() {
  return COMMUNITY_GROUPS.flatMap((group) =>
    (getCommunity(group)?.posts || []).map((p) => ({ group, postSlug: p.slug }))
  );
}

export async function generateMetadata({ params }: { params: Promise<{ group: string; postSlug: string }> }) {
  const { group, postSlug } = await params;
  const post = getPost(group, postSlug);
  return { title: post ? `${post.title} · Community` : "Post" };
}

export default async function PostPage({ params }: { params: Promise<{ group: string; postSlug: string }> }) {
  const { group, postSlug } = await params;
  const post = getPost(group, postSlug);
  if (!post) notFound();

  const community = getCommunity(group)!;
  const label = getLabel(group, post.labelId);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-[1100px] mx-auto px-6 py-6 flex gap-6">
        <div className="min-w-0 flex-1">
          <Link href={`/community/${group}`} className="text-[13px] text-muted hover:text-foreground inline-flex items-center gap-1.5 mb-4">
